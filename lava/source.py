...
class BilibiliSource(BaseSource):
    ...
    def check_query(self, query: str) -> bool:
        """
        Check if the query is a valid Bilibili video URL.
        
        :param query: The input query to check
        :return: True if the query is a Bilibili video URL, False otherwise
        """
    ...
    def get_video_info(bvid: str) -> Tuple[str, str]:
        """
        Get Bilibili video information for the given video ID.

        :param bvid: The Bilibili video ID
        :return: A tuple containing the video's CID and session string
        """
        headers = {
